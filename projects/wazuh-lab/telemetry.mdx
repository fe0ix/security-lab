---
title: "Telemetry"
description: "In this phase, I focused on enhancing the telemetry collected from the endpoints to enable more effective detection. I implemented Sysmon for Windows and Linux to capture detailed process and network activity and configured Docker event collection."
---
...
<Frame caption="Command detection after Sysmon installation">
  ![Command detection after Sysmon installation](./assets/screenshots/common-discovery.png)
</Frame>


For comparison, I ran similar commands on my workstation `home-home-paw-01` before installing Sysmon and could not identify equivalent telemetry in Wazuh. This confirmed that without Sysmon, Wazuh has limited visibility into process-level / powershell activity on Windows by default.

<Frame caption="Test command overview">
  ![Test command overview](./assets/screenshots/sysmon-windows.png)
</Frame>
...
<Frame caption="Centralized Sysmon configuration result">
  ![Centralized Sysmon configuration result](./assets/screenshots/centralized-sysmon-config.png)
</Frame>
...
<Frame caption="AD audit policy configuration">
  ![AD audit policy configuration](./assets/screenshots/auditpolicy-settings-gpo.png)
</Frame>
...
<Steps>
  <Step title="Install Sysmon for Linux">
    Install Sysmon for Linux following the [installation instructions](https://github.com/microsoft/SysmonForLinux/blob/main/INSTALL.md).
  </Step>

  <Step title="Update the Linux group agent.conf">
    Add the syslog log source to collect Sysmon output:
...
```
  ...
  <Step title="Add a custom decoder">
    I used the decoder from the Wazuh [Koske malware blog post](https://wazuh.com/blog/detecting-koske-malware-with-wazuh/). It parses the XML-structured Sysmon output from syslog and extracts fields like `EventID`, `Image`, `CommandLine`, `DestinationIp`, and `TargetFilename` into named fields that rules can match on.

```xml sysmon decoder expandable
...
```
  ...
  <Step title="Add base detection rules">
    These rules map each Sysmon Event ID to a Wazuh alert and serve as the foundation for the more specific detections on the [Detection Rules](/projects/wazuh-lab/detection-rules) page. The override rules (200200–200202) suppress noise from the Wazuh agent's own file and network activity.

```xml detection rule expandable
...
<Frame caption="Sysmon for Linux Event validation">
  ![Sysmon for Linux Event validation](./assets/screenshots/sysmon-linux.png)
</Frame>
...
<Warning>
At the time of implementation, Wazuh `v4.14.3` had a bug where [not all Docker events triggered alerts](https://github.com/wazuh/wazuh/pull/34376#pullrequestreview-3762003381). For example, container stop and pause events were not visible. Because the fix was only available in the `v4.14.4` release candidate, I did not move to the pre-release version. Instead, I manually applied the rule changes by replacing `ruleset/rules/0560-docker_integration_rules.xml` with the updated [version from the related pull request](https://github.com/wazuh/wazuh/pull/34376/changes/8dc43bf1b3fc20996495483e325a27f785bdb2e6#diff-9bc8c9f333f88951c84e1195a8cdd9e259958fe40c1ef3d7132ea8698de67d12).
</Warning>


<Frame caption="Docker Event validation">
  ![Docker Event validation](./assets/screenshots/docker-event-test.png)
</Frame>
...
<Frame caption="CIS Docker Benchmark results">
  ![CIS Docker Benchmark results](./assets/screenshots/docker-cis.png)
</Frame>

...
description: "This page compares the two SIEM environments I have set up as part of this portfolio. It focuses more on the pros and cons and my own observations based on personal experience than just a theoretical product comparison."
---
...
<Tabs>
  <Tab title="Wazuh">
    - Detection rules are XML-based and match against decoded log fields — my [SharpHound](/projects/wazuh-lab/detection-rules#sharphound-detection) and [PowerShell abuse](/projects/wazuh-lab/detection-rules#powershell-activity-detection) rules each needed 30\+ lines of code
    ...
    When a detection doesn't work properly, troubleshooting means working backwards through each layer to find where it breaks — which was the most time-consuming part of building detections in Wazuh
...
<Tabs>
  <Tab title="Wazuh">
    ...
    - At production scale, certificate rotation, system tuning, backup configuration, and storage/disk management become ongoing tasks
  </Tab>
  <Tab title="Microsoft Sentinel">
...
For a security team with 1–2 members, Sentinel’s managed infrastructure is a clear advantage. For teams that require full control over data storage due to compliance policies, etc.—or that operate in an air-gapped environment — Wazuh or another on-premises solution would be the better choice. As someone who values open source, I was impressed by how much Wazuh has grown—features like [Wazuh CTI](https://wazuh.com/blog/introducing-wazuh-cti/) and [IT hygiene](https://wazuh.com/blog/improving-it-hygiene-using-wazuh/) monitoring show that you no longer necessarily need a commercial solution.

\

_Translated with [DeepL.com](http://DeepL.com) (free version)_
